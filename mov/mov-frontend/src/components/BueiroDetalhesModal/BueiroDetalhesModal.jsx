...
function BueiroDetalhesModal({ bueiro, onClose }) {
  ...
  useEffect(() => {
    const carregarEventos = async () => {
      ...
      if (response.ok) {
        // Filtrar eventos deste bueiro específico
  ...
  // Calcular dias desde a última atualização (simulado - usando data de criação ou último evento)
  const calcularDiasAtualizacao = () => {
    ...
    // Se não houver eventos, retorna um valor padrão (7 dias)
    return 7;
  };

  const diasAtualizacao = calcularDiasAtualizacao();

  // Formatar data de instalação (simulado - usando data de criação do bueiro ou padrão)
  const formatarDataInstalacao = () => {
    // Se houver uma data de criação no bueiro, usar ela
    // Caso contrário, usar uma data padrão
  ...
  const device = bueiro.device || `ESP32-${bueiro.id.toString().padStart(6, "0")}`;
      ...
      <div className="detalhes-card status-card">
        ...
        <div className="status-card-content">
          <div className="status-badge-container">
            <span className={`status-badge-large ${getStatusColor(bueiro.status)}`}>
              <svg
          ...
          <div className="status-update-info">
            Atualizado há {diasAtualizacao} {diasAtualizacao === 1 ? "dia" : "dias"}
          </div>
        </div>
      </div>
          ...
          <div className="info-column">
            ...
            <div className="info-row">
              <span className="info-label">
                ...
                {parseFloat(bueiro.latitude).toFixed(6)}, {parseFloat(bueiro.longitude).toFixed(6)}
              </span>
            </div>
            <div className="info-row">
              <span className="info-label">
