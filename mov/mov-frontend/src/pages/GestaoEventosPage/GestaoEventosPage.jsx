...
function GestaoEventosPage() {
  ...
  const eventosFiltrados = eventos.filter((evento) => {
    const matchStatus =
      filtroStatus === "todos" ||
      normalizeString(evento.statusAtendimento) === normalizeString(filtroStatus);
  ...
  // Função para obter a cor da barra lateral baseada na prioridade e status
  const getBarraColor = (prioridade, statusAtendimento) => {
    if (statusAtendimento === "Fechado") return "#E0E0E0"; // Cinza
    if (prioridade === "Alta") return "#D32F2F"; // Vermelho
    if (prioridade === "Média") return "#FF7F00"; // Laranja
    return "#E0E0E0"; // Cinza para Baixa
  };
            ...
            >
              <div className="evento-content">
                ...
                <div className="evento-chips">
                  <span className={`chip ${getPrioridadeColor(evento.prioridade)}`}>
                    {evento.prioridade}
                  </span>
                  <span className={`chip ${getStatusColor(evento.statusAtendimento)}`}>
                    {evento.statusAtendimento}
                  </span>
                </div>
              </div>
