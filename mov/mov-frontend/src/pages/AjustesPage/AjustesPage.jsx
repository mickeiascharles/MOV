...
// 1. CORREÇÃO: Removemos 'getMeuPerfil' (por enquanto) e importamos 'uploadFotoPerfil'
import {
  getMeuPerfil,
  updateMeuPerfil,
  uploadFotoPerfil,
...
// 2. CORREÇÃO: Removemos o import do 'placeholderAvatar' que não era usado
...
function AjustesPage() {
  ...
  // 3. CORREÇÃO: Corrigido o useEffect
  useEffect(() => {
    const carregarPerfil = async () => {
      if (!currentUser) return; // Se não há usuário, não faça nada
      ...
      if (response.ok) {
        // NÃO chame login() aqui (isso causa o loop)
    ...
    carregarPerfil();
  }, [currentUser]); // 4. CORREÇÃO: A dependência é 'currentUser'

  // Salva os DADOS (nome, email, contato)
  const handleSalvar = async (e) => {
    ...
    if (response.ok) {
      // Aqui SIM chamamos login(), para atualizar o localStorage
      login(response.data);
  ...
  const handleFileChange = async (event) => {
    ...
    if (file) {
      ...
      if (response.ok) {
        // Atualiza o usuário no Contexto com a nova foto_url
        login(response.data);
    ...
    <div className="gestao-container">
      ...
      <div className="widget-card form-card">
        ...
        <div className="perfil-foto-wrapper">
          <img
            // 5. CORREÇÃO: Usamos 'currentUser.foto_url' (do Contexto)
            // para a imagem carregar instantaneamente após o upload
            src={
