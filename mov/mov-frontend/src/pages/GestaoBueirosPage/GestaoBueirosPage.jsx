...
// Componente para ajustar o zoom do mapa baseado nos bueiros
function MapBounds({ bueiros }) {
  const map = useMap();
  
  useEffect(() => {
    if (bueiros.length > 0) {
      const bounds = bueiros.map(b => [parseFloat(b.latitude), parseFloat(b.longitude)]);
      if (bounds.length === 1) {
...
function GestaoBueirosPage() {
  ...
  // Centro padrão (Brasília) se não houver bueiros
  const defaultCenter = [-15.793889, -47.882778];
  const mapCenter = bueiros.length > 0 
    ...
    <div className="gestao-bueiros-page">
      ...
      <div className="gestao-bueiros-header">
        <div className="header-text">
          ...
          <p className="header-subtitle">Gerencie todos os bueiros monitorados</p>
        </div>
        <button 
      ...
      <div className="mapa-section">
        <div className="mapa-header-section">
          ...
          <p className="mapa-subtitle">
            Localização geográfica de todos os bueiros ({bueiros.length} no total)
          </p>
        </div>
          ...
          {isLoading ? (
            ...
            >
              ...
              {bueiros.map((bueiro) => (
                <Marker
                  key={bueiro.id}
                  position={[parseFloat(bueiro.latitude), parseFloat(bueiro.longitude)]}
                >
                  <Popup>
            ...
            {bueiros.map((bueiro) => (
              <div key={bueiro.id} className="bueiro-card">
                <div className="bueiro-card-header">
                  ...
                  <span className={`status-badge ${getStatusColor(bueiro.status)}`}>
      ...
      <Modal isOpen={selectedBueiro !== null} onClose={() => setSelectedBueiro(null)}>
        {selectedBueiro && (
          <BueiroDetalhesModal
